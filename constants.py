...
SUPPORTED_EXTENSIONS = {
    ...
    # ".txt"ファイルの読み込みを追加。
    # encodeingを指定しないと、cp932（Shift_JIS系）がデフォルトで設定されてしまいエラーになる。
...
# ==========================================
# 表示位置の振り分けはサイドバーで完結したのでカラムは不要だったが、覚えておくべき内容なので残す

# 注意：エラー発生原因となるので、ここでカラムを定義してはいけない。
# 最初に実行されるStreamlitコマンドはst.set_page_config()でなければならない。
# ==========================================
# col1, col2 = st.columns([20, 80])　← 必要ならcomponents.pyの各関数毎に定義する